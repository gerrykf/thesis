...
需求的衍生： 
1. 在首页显示今天不知道锻炼什么运动？
2. 点击提示，可以通过转盘随机选择一种运动方式进行锻炼。
3. 转盘上可以显示多种运动方式，如跑步、跳绳、瑜伽等。
4. 转盘动画开始转动，最终停在某个运动方式上，提示用户进行该运动锻炼。
5. 将抽中行的运动保存到本地 用户打开打卡页面时 运动类型那一栏默认显示为抽中的运动方式。

轮盘动画：
实现使用CSS动画和JavaScript控制转盘的旋转效果。
要求做成时序动画：
1. 轮盘从底部从小到大显示到屏幕中央
2. 转盘开始转动时逐渐加速，然后减速，最后停在指定位置
3. 转盘停止后显示抽中的运动方式,再弹窗提示用户进行该运动锻炼。
4. 用户点击确认，最后轮盘从屏幕中央逐渐缩小消失到底部。