    ...
    <div class="content">
      ...
      <div class="stats-card">
        <div class="stats-header">
          <span class="title">{{ currentMonth + 1 }}月打卡进度</span>
          <span class="count">{{ monthStats.checkedDays }} / {{ monthStats.totalDays }}</span>
        </div>
        <van-progress
      ...
      <div class="calendar-display">
        ...
        <div class="month-header">
          <van-icon name="arrow-left" @click="prevMonth" :class="{ disabled: !canGoPrev }" />
          <div class="month-title">{{ currentMonthTitle }}</div>
          <van-icon name="arrow" @click="nextMonth" :class="{ disabled: !canGoNext }" />
        </div>
...
const checkedRate = computed(() => {
  ...
  return Math.round((monthStats.value.checkedDays / monthStats.value.totalDays) * 100)
})
...
async function loadRecords() {
  try {
    ...
    // 尝试不同的数据路径
    let records = response.data?.data?.records || (response.data as any)?.records || []
