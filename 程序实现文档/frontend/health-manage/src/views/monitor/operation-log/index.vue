...
import { ref } from "vue";
import { useOperationLogs } from "./hook";
import { PureTableBar } from "@/components/RePureTableBar";
import { useRenderIcon } from "@/components/ReIcon/src/hooks";
import { OperationLogPermission } from "@/utils/rbac";
...
} = useOperationLogs(tableRef);

// 获取日期快捷选项
const getPickerShortcuts = () => {
  return [
    {
      text: "最近一周",
      value: () => {
    ...
    <PureTableBar title="操作日志列表" :columns="columns" @refresh="onSearch">
      <template #buttons>
        <el-popconfirm title="确定要删除所有日志数据吗？" @confirm="clearAll">
          <template #reference>
            <el-button
              v-perms="[OperationLogPermission.CLEAR_ALL]"
              type="danger"
      ...
      <template v-slot="{ size, dynamicColumns }">
        <div
          v-if="selectedNum > 0"
          v-perms="[OperationLogPermission.PATCH]"
          v-motion-fade
