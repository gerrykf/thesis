...
1. 程序实现文档/backend/health-backend/src/controllers/foodController.ts 
    > getFoods 这里 H5 会在请求头中携带单签语言 然后 服务端根据语言返回不同语言的食物字段
    > 但是我希望后台管理端将所有字段都返回
我现在能想到的解决方案就是 h5端一个接口 后台管理端一个接口 差距只在要不要返回所有语言的字段于返回的字段